...
class TestDataSetDefine(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # --------------- Read Image ------------------
        img_path = os.path.join(self.cfg.test_img_dir, img_name)
...
class TestDataSet(object):
    def __init__(self, config):
        ...
        self.dataset = TestDataSetDefine(self.item_list, self.d2id, config)
        self.loader = data.DataLoader(dataset=self.dataset, batch_size=self.cfg.test_batch_size,
                                      shuffle=False, num_workers=10)
        self.batches = self.loader.__len__()

        print('Testing:(%d batches) (%d samples)' % (self.batches, self.samples))
        print('===========================================================\n')
